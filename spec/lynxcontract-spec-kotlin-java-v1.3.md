...
> production adapter: fill tokens, stub realization modes, multipliers, provenance markers,
...
> **v1.1** folds in field-proven patterns from a production contract-first module (a production TypeScript platform: its "Atlas" module + its `enrichers` ETL registry; names fictionalized):
> `INTENT`/`BUSINESS LOGIC` doc shape, **realizes/realizedBy** contract↔file mapping (§7–§8), a dedicated **module-composition graph** block `//@graph:` (§12.1), an **observability contract** `//@observability:` (§14.1), a **plugin/registry** block `//@plugin:` (§12.2), inline `mermaid` in contracts (§14), topic/channel **constant helpers** and **freeze / closed-enum** rules for schema evolution (§13, §19.1).
...
| `//@observability:` structured-logging contract | §14.1 | Atlas "Observability contract" — mandated log fields + `outcome` enum + `duration_ms` |
...
| **Observability** *(v1.1)* | `//@observability:` | operation / service / class | mandated structured-log fields, `outcome` enum, `duration_ms` (§14.1) |
...
//@    debit path; batch debit has its own contract.
...
//@    - Reservations are idempotent per requestId; a redelivery MUST NOT double-debit.
...
//@  pre: command.id != null && command.currency in SUPPORTED_CURRENCIES
//@  post: result.deviceId != null && result.status == "OPEN"
//@  assigns: []                       # pure orchestration, no local state
//@  raises:
//@    PermanentException: command.currency !in SUPPORTED_CURRENCIES
...
//@  key: providerTicker                                          # what selects a module at runtime
//@  members: [alphaspot, betapool, gammafx, deltabank, ...]                # registered modules (source of truth)
...
**Rule:** `members` here, the entries in `registry`, and the folders on disk MUST stay in sync — a member without a registry line (or vice-versa) is a lint error. Documenting per-module special-casing (regions, flags, quirks) in one table — as Atlas does with `PROVIDER_QUIRK_TABLE` — keeps DRY and tells codegen where the branches are.
...
| `format` | enum | `envelope-json` (JSON `{metadata, payload}`, payload is a **stringified** JSON), `avro` (via registry), `json` (raw). |
...
| `headers` | list | Required message headers (correlation-id, signature, core-identifier, …). |
...
The Atlas module never concatenates channel names inline (`atlasDomainEventChannel(type)` helpers); "a typo becomes a compile error." Apply the same to Kafka:
...
//@    - "emitSse: { operation:'emitSse', channel, event, feedId, tenantName }"
//@  mustNotLog: [rawSecrets, keys.*, per-subscriber delivery counts]  # forbidden / unknown-as-known
fun openDevice(...) { ... }
```
...
### 18·2 Formats
- `envelope-json` = `messaging-commons` `Envelope { metadata, payload }`, **`payload` is a stringified JSON** (double-encoded). Requires a CDI `ObjectMapper` bean.
...
### 18·3 Error routing (the retry library)
...
`acme-correlation-id` (in `metadata.properties`), `acme-core-identifier`, `acme-event-external-timestamp`, `acme-event-ingestion-timestamp`, signature set `x-signature`/`x-alg`(Ed25519)/`x-key-id`/`x-ts`/`x-sig-v`/`x-nonce`.
...
//@    values: [ok, syncing, error, "on hold"]   # 'on hold' keeps the literal space
```
...
explicit **variation point**. This section standardizes the notation layer proven in production
template authoring (two full isolated instantiation runs against a real adapter). A template is
...
© 2025–2026 LynxContract Authors. MIT License. JVM edition (v1.3) derived from the Python/Go spec v0.2 (core) + v0.3 (concurrency), extended with patterns from a production TypeScript platform (its "Atlas" module and `enrichers` ETL registry; names fictionalized), and (v1.2) with the archetype/template layer proven by six isolated battle runs of a production adapter-family template.
