...
test('rule: frozen/closed entries become enum_surface nodes with freezes edges (§19.1)', () => {
  ...
  assert.deepEqual(surface.attrs.values, ['OPEN', 'COMPLETE', 'IN REVIEW']);
...
test('spec §6.1 (v1.0): CODEOWNERS gitignore-glob matches module root paths', () => {
  ...
  assert.ok(codeownersMatch('/services/**', 'relay', 'services/relay'));
  assert.ok(codeownersMatch('services/*', 'relay', 'services/relay'));
  assert.ok(codeownersMatch('**/relay/', 'relay', 'org/services/relay'));
  assert.ok(codeownersMatch('services/re?ay/', 'relay', 'services/relay'));
  // non-matches: anchored elsewhere, different subtree
  assert.ok(!codeownersMatch('/other/**', 'relay', 'services/relay'));
  assert.ok(!codeownersMatch('services/notify/', 'relay', 'services/relay'));
  assert.ok(!codeownersMatch('relayx', 'relay', 'services/relay'));
});
