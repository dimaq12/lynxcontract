...
| v1.1 | 2026 | Field-proven patterns from a production TypeScript platform (fictionalized as "Atlas"): `INTENT`/`BUSINESS LOGIC` doc shape, `realizes`/`realizedBy` realization map, `//@graph:`, `//@plugin:` registries, `//@observability:`, inline mermaid, topic-constant helpers, freeze/closed-enum rules (§19.1). |
...
Six isolated instantiation runs against a production codebase: each run was a fresh agent
...
defect). Product details are out of scope here by policy; what shaped the spec was the *classes*
of failure:
