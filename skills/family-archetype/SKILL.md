...
(§20 is the normative template layer). This skill *produces the archetype*;
the `instantiation-run` skill then *executes a run of it* (plan → waves →
self-lint → diff), with `contract-first` bracketing the run (Phase 0 sync in,
Phase 3 contract-test out).
...
- No baked-in domain field, no provider-specific endpoint, no member-specific
  ...
  `PROVIDER_OVERRIDES` map) is allowed **only** as a declared, per-member
...
3. Execute the run end to end with the **`instantiation-run`** skill: run plan
   *before any code* (waves from `generationOrder` §20.5, realization modes,
   declared counts, exclusions and deferrals) → generation waves under the
   honesty protocol → mechanical self-lint (§20.8) → adversarial diff and run
   report.
4. The **`contract-first`** skill brackets the run: Phase 0 sync feeds the
   plan, and the **Phase 3 contract-test** against the real/recorded provider
   call closes it. The provider fill is the one irreducible per-member
   variable, and Phase 3 is what proves it right (a hallucinated endpoint
   fails the recorded stub even on a green build).
...
## Appendix — what an archetype looks like on disk (any language)

The shape is language-neutral and deliberately small: **one family-law stub +
one stub per future file + test stubs**, nothing else. Every `.lynx.*` file is
a contract for a file that does not exist yet, 1:1 with the tree it generates.
Four fictional families, one per host language:

```
template-notify/                      # Kotlin — event-consumer family
├── 00-family.lynx.md                 # //@graph: inventory + depends + generationOrder (§20.5)
├── NotifyRoute.lynx.kt               # the fixed entry point of every member
├── {{Channel}}Sender.lynx.kt         # MULTIPLIER: one sender per channel instance (§20.3)
└── NotifyRouteShould.lynx.kt         # one //@contract: per test case

template-relay/                       # Go — forwarding-worker family
├── 00-family.lynx.md
├── relay_route.lynx.go               # #@ markers in Go comment form
├── {{hop}}_forwarder.lynx.go         # MULTIPLIER over declared hops
└── relay_route_test.lynx.go

template-analytics/                   # Python — report-generator family
├── 00-family.lynx.md
├── report.lynx.py
├── {{metric}}_extractor.lynx.py      # MULTIPLIER over the metric roster
└── test_report.lynx.py

template-sensor/                      # Rust — signal-scorer family
├── 00-family.lynx.md
├── signal_scorer.lynx.rs
├── {{signal}}_model.lynx.rs          # MULTIPLIER over signal kinds
└── signal_scorer_test.lynx.rs
```

What all four share — and all an archetype ever needs:

- **`00-family`** carries everything family-wide: the file inventory (== the
  stub tree), `depends`, the dataflow sketch, `generationOrder`, and the
  family's frozen vocabularies (§19.1).
- **One stub per future file**; a fixed file is a plain stub, a repeating file
  is a multiplier stub whose roster lives in the manifest (§20.6).
- **Tests are stubs too** — assertions are contract-derived, never invented.
- Layout beyond this is a *fill*, not archetype: package roots, directory
  depth, build files — anything two families could reasonably disagree on
  stays out of the pattern.

A file that exists in a generated member but has no stub, no multiplier
instance, and no gap-ledger entry is drift by construction (§20.8-1).
