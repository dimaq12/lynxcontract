...
//@  pre: command.windowMs > 0
//@  post: result.captureId.isNotBlank()
//@  calls: [telemetryApi.create]
//@  raises:
//@    PermanentException: provider rejects the capture request
//@  assigns: []
```
...
//@  lang: kotlin
//@  signature: data class {{Command}}(val id: String, val deviceId: String, val windowMs: Long)
...
**From manifest to run plan.** Before the first generated line, the manifest and the
template's `generationOrder` (§20.5) compile into a **run plan** — a scratchpad artifact,
never committed, never cited from generated code:

```markdown
# Run plan — acme-telemetry @ corelab (scratchpad)

Phase 0 record: contracts synced against event-specs@<sha>
Expected targets: 4 generated + 1 copy-verbatim = 5 files

| Wave | Targets | Mode |
|---|---|---|
| 1 | commands/{StartCapture,StopCapture} | generate |
| 2 | CaptureMapper | copy-verbatim |
| 3 | StartCaptureRoute | generate |
| 4 | StartCaptureRouteShould | generate |

Excluded: ResetCapture — BLOCKED, upstream schema unpinned (manifest §Blocked)
Deferred: mapper bodies needing live provider access — declared now, so the
closing diff classifies them as predicted, not defects
```

The plan is where honesty starts, not the code: ambiguities met while planning go straight
to the gap ledger (planning gaps are first-class), a stub↔manifest roster disagreement found
here triggers the §20 roster-disagreement rule instead of a silent guess, and the §20.8
inventory check later holds one bijection across all three lists — stub tree == inventory ==
plan entries. Plan bookkeeping (wave labels, gap ids) stays in the scratchpad; marker
hygiene (§20.7) forbids it in generated code. The full run protocol — plan, waves,
self-lint, closing diff — is the `skills/instantiation-run` skill.
...
entry is drift — recipe 17). Later, an adversarial diff classifies every divergence as
